(function() {
    ...
    require_modules[33] = function(global, module, require) {
        ...
        var DataTableHeader = view.newClass("DataTableHeader", Base, {
            template: fun.newProp("template"),
            _template: '<table class="uki-dataTable-header" style="{{style}}"><tbody><tr class="uki-dataTable-header-row">{{#columns}}<td class="uki-dataTable-header-cell {{className}}" style="{{style}}"><div class="uki-dataTable-header-wrap"><div class="uki-dataTable-header-text">{{label}}</div><input type="input" name="{{filter}}" style="{{filterstyle}}" tabindex="1" class="uki-dataTable-filter"><div class="uki-dataTable-resizer uki-dataTable-resizer_pos-{{pos}}">|</div></div></td>{{/columns}}</tr></table>',
